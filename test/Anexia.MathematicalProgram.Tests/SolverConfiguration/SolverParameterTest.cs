...
{
    [Fact]
    public void TimeLimitConstructorDisablesSolverOutputByDefault()
    {
        var parameters = new SolverParameter(new TimeLimitInMilliseconds(2_500));

        Assert.Equal(EnableSolverOutput.False, parameters.EnableSolverOutput);
    }

    [Fact]
    public void TimeLimitConstructorLeavesRelativeGapUnset()
    {
        var parameters = new SolverParameter(new TimeLimitInMilliseconds(2_500));
    ...
    [Fact]
    public void TimeLimitConstructorKeepsGivenTimeLimit()
    {
    ...
    [Fact]
    public void TimeLimitConstructorLeavesNumberOfThreadsUnset()
    {
        var parameters = new SolverParameter(new TimeLimitInMilliseconds(2_500));
    ...
    [Fact]
    public void TimeLimitInMillisecondsConvertsToTruncatedSeconds()
    {
        var timeLimit = new TimeLimitInMilliseconds(2_500);

        Assert.Equal(2u, timeLimit.AsSeconds);
    }
    ...
    [Theory]
    [InlineData(LpSolverType.Glop, "num_omp_threads:3,custom:value")]
    [InlineData(LpSolverType.Scip, "parallel/maxnthreads=3,custom=value")]
    public void ToSolverSpecificParametersForLpUsesSolverSpecificSeparators(
        LpSolverType solverType,
        string expected)
    {
        var parameters = new SolverParameter(
        ...
        Assert.Equal(expected, parameters.ToSolverSpecificParameters(solverType));
    }
}

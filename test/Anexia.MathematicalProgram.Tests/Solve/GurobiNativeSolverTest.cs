...
{
    [Fact]
    public void SolveWrapsGurobiSetupExceptionMessage()
    {
        var (exception, _) = SolveWithInvalidGurobiParameterAndLogger();

        Assert.Contains("Error in solver:", exception.Message);
    }

    [Fact]
    public void SolveWrapsGurobiSetupExceptionWithInnerException()
    {
        var (exception, _) = SolveWithInvalidGurobiParameterAndLogger();
    ...
    [Fact]
    public void SolveLogsErrorWhenGurobiSetupFails()
    {
        var (_, logger) = SolveWithInvalidGurobiParameterAndLogger();

        Assert.Equal(LogLevel.Error, logger.LastLogLevel);
    }

    [Fact]
    public void SolveLogsGurobiSetupExceptionAsLastException()
    {
        var (exception, logger) = SolveWithInvalidGurobiParameterAndLogger();

        Assert.Same(exception.InnerException, logger.LastException);
    }

    [Fact]
    public void SolveWithoutLoggerWrapsGurobiSetupExceptionMessage()
    {
        var exception = SolveWithInvalidGurobiParameterWithoutLogger();

        Assert.Contains("Error in solver:", exception.Message);
    }

    [Fact]
    public void SolveWithoutLoggerWrapsGurobiSetupExceptionWithInnerException()
    {
        var exception = SolveWithInvalidGurobiParameterWithoutLogger();
    ...
    public void SolverWithSimpleFeasibleIlpModelReturnsCorrectResult()
    {
        ...
        var result = new IlpSolver(IlpSolverType.GurobiNativeIntegerProgramming).Solve(optimizationModel,
    ...
    public void SolverWithoutLicenceLogsFallbackSolverName()
    {
        var (_, logger, _) = SolveSimpleModelWithScipFallback();

        Assert.Contains(logger.Messages, message => message.Contains("Scip"));
    }

    [RequiresNoGurobiLicenceFact]
    public void SolverWithoutLicenceLogsMissingLicence()
    {
        var (_, logger, _) = SolveSimpleModelWithScipFallback();
    ...
    {
        var (result, _, variable) = SolveSimpleModelWithScipFallback();

        Assert.Equal(
            SolverResult(
                SolutionValues<IIntegerVariable<IRealScalar>, RealScalar, IRealScalar>(
                    (variable, new RealScalar(1))), new ObjectiveValue(2), new IsFeasible(true),
                new IsOptimal(true), new OptimalityGap(0),
                SolverResultStatus.Optimal, false), result);
    }
    ...
    public void GurobiWithoutORToolsGivesSameResultAsWithORTools()
    {
        ...
        var resultGurobiAPI = new IlpSolver(IlpSolverType.GurobiNativeIntegerProgramming).Solve(
            optimizationModel,
            new SolverParameter(new EnableSolverOutput(false), RelativeGap.EMinus7,
    ...
    private static (MathematicalProgramException Exception, FakeLogger Logger)
        SolveWithInvalidGurobiParameterAndLogger()
    {
    ...
    private static MathematicalProgramException SolveWithInvalidGurobiParameterWithoutLogger() =>
    ...
    private static (ISolverResult<IIntegerVariable<IRealScalar>, RealScalar, IRealScalar> Result,
        FakeLogger Logger,
        IIntegerVariable<IRealScalar> Variable) SolveSimpleModelWithScipFallback()
    {
        /*
         * min 2x, s.t. x=1, x binary
         */
